...
**최후 수단 채널 — みんせつ (minnano-setsumeikai, 기관투자자 IR 플랫폼):**

일본에서 기관투자자·상장사 간 IR 일정 조정에 널리 쓰이는 플랫폼. 약 3,000개 상장사의
결산설명회 등록·그룹 미팅 신청·1:1 미팅 요청·일정 조정을 지원한다 (JPX도 공식 소개).

- **접촉 우선순위: IR 이메일 주소(콜드메일) > IR 문의 웹폼(`ir-webform`) > みんせつ.**
  이메일이나 웹폼이 하나라도 있으면 그 경로로 진행하고 みんせつ는 쓰지 않는다.
  둘 다 끝내 못 찾은 경우에만 みんせつ에서 대상 회사 등재 여부를 확인해 활용한다.
- 계정 유무는 레지스트리(api-key-registry)에서 'みんせつ/minnano-setsumeikai' 조회로
  확인. **미등록 상태면 이 스킬에서 가입을 진행하지 않는다** — 한국 소재 운용사의
  등록 요건·비용은 별도 문의가 필요하므로, "みんせつ 기관 등록 검토"를 결과 보고에
  1줄 언급만 한다.
- みんせつ 사이트 내 미팅 신청·폼 제출은 웹폼 제출이므로 사전 확인 대상
  (common-rules 되돌릴 수 없는 작업 원칙).
