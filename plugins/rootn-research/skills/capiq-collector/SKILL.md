...
## 1. 수집 대상 — 기본 수집 세트(요청 없어도 전부 수집)

> **정책**: 종목이 지정되면 아래 A(기본 수집)를 **요청 없이 전량 수집**한다. 각 항목은 실패해도 다음 항목으로
> 넘어가고 개별 보고(§3 fail-fast). B(route-only)는 기본 스킵하되 route/존재만 기록, 명시 요청 시 수집.

### A. 기본 수집 (default, 요청 불요)
| 데이터 | 방법 | Drive 목적지 |
|---|---|---|
| **Earnings call transcripts** (최근 2년, Q&A 포함) | Documents/Transcripts → Word/PDF 다운로드 | `IR/transcripts/` |
| **셀사이드 리포트** (Aftermarket, entitlement 범위) | Research 탭 브로커 PDF. **미국·중국 우선** | `리서치/` |
| **컨센서스 상세** (Detailed Estimates) | `Export to Excel as Data`(렌더러 안정 시 파일, **기간 2026 고정**) | `리서치/consensus/` |
| **forward 컨센서스** (Estimate Highlights FY+1~+3 EPS·TP·Multiples) | `estimateHighlights?ID=` **JS innerText 스크랩** → xlsx | `리서치/consensus/` |
| **Ownership Detailed** (주주·기관·액티비스트·인사이더) | `PublicOwnershipDetailed?ID=` **JS `<table>` 스크랩**(이름/지표 index 조인) → xlsx | `리서치/ownership/` |
| **Analyst Coverage** (커버 애널리스트·투자의견·TP 분포) | `analystCoverage?ID=` **JS `<table>` 스크랩** → xlsx | `리서치/consensus/` |
| **Corporate Structure** (지분법·자회사·모자 관계) | `corporateStructure?Id=` **JS `<table>` 스크랩** → xlsx | `리서치/` |
| **Key Developments** (이벤트 타임라인) | `keyDevelopments` **JS `<table>` 스크랩**(존재 확인 후) → xlsx | `리서치/` |

### B. route-only (기본 스킵, 명시 요청 시 수집)
| 데이터 | route | 비고 |
|---|---|---|
| Business Relationships / Supply Chain | (네비 onClick, 직접 URL) | `<table>` 유무 미검증 |
| Transactions (M&A·자금조달 이력) | (네비 onClick) | 미검증 |
| Corporate Issuance (발행 이력) | (네비 onClick) | 미검증 |
| Peer Comps | `peerAnalyticsMain?id=` | 커스텀 그리드(비-`<table>`)·export X → 자동 비신뢰 |

> ★ **신규 섹션은 실행 시 `<table>` 유무를 먼저 확인**하고, 없으면(커스텀 그리드) 스킵·기록(억지 스크랩 금지).
> **CapIQ Excel export 신뢰도 상이**: Detailed Estimates=파일 O, Estimate Highlights·Peer Comps=다운로드 X → **JS 스크랩 1차**.
> **Financials**는 CreditStats Direct(iframe)라 자동 수집 실패 → 숫자는 Bloomberg BEst·EDINET로 대체.
> **MI KEY 해석**: 상단 Search 자동화 불가 → `references/capiq-mikey-registry.md` 캐시 사용(§2-1).
> 신규 섹션 상세 route·방법은 references/capiq-paths.md §9 참조.
...
### 2-1. 종목 식별 (MI KEY 해석 — 최소입력·비차단)
- **상단 Search 자동화 불가**(값 주입 리셋·백엔드 gRPC·소스 CORS 차단, capiq-paths §9). 자동 룩업 시도 금지.
- **해석 순서(비차단)**: (1) `references/capiq-mikey-registry.md` 조회 → 있으면 그 MI KEY로 직행(입력 0).
  (2) 미스면 **멈추지 않는다**(공통규칙 §2 ask-async): 해석되는 종목부터 **먼저 전부 수집**하고,
  미해석 종목들은 **한 목록으로 모아** "CapIQ에서 열고 URL(`.../profile?id=<MIKEY>`) 또는 MI KEY를 달라"고
  **결과 보고에 함께** 요청한다(순수 식별 모호 시 동명·거래소도 함께). 응답이 없으면 그 종목만 스킵하고
  나머지·다른 태스크를 계속하며, 스킵분은 개별 보고(종목·필요입력·사유). **하나 때문에 전체를 대기시키지 않는다.**
- 확보한 MI KEY는 **즉시 registry에 append**(티커·회사명·MI KEY·SPCIQ·확보일) 후 SSOT push → 재입력 불필요.
